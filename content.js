...
function translatableBlocks(root = rootNode()) {
  ...
  const title = articleTitleNode();
  return [...new Set([title, ...root.querySelectorAll(`${BLOCKS}, [data-llt-quiz-prompt='1']`)].filter(Boolean))];
}
...
function render(element, text, key, protectedItems = []) {
  ...
  output.classList.toggle("llt-title-translation", element === articleTitleNode());
...
function articleTitleNode() {
  const article = document.querySelector(".article-inner");
  return article?.querySelector(".content-title, h1, h2, .article-title, [class*='article-title']") || null;
}

function articleTitle() {
  const article = document.querySelector(".article-inner");
  const heading = articleTitleNode();
...
function articleTitleInfo() {
  const element = articleTitleNode();
  const original = articleTitle();
  const output = element?.nextElementSibling?.classList.contains("llt-translation") ? element.nextElementSibling : null;
  const translated = output?.innerText.replace(/\s+/g, " ").trim() || original;
  return { element, original, translated };
}
...
function buildArchive() {
  const titleInfo = articleTitleInfo();
  const title = titleInfo.translated;
  const info = courseInfo();
  const blocks = translatableBlocks().filter((element) => element !== titleInfo.element && element.dataset.lltDone === "1");
  const archiveNodes = staticArchiveNodes(rootNode(), blocks);
  const markdown = [`# ${title}`, "", `> 原文标题：${titleInfo.original}`, `> Source: ${canonicalUrl()}`, `> Saved: ${new Date().toISOString()}`, ""];
  const htmlBlocks = [];
  ...
  if (mediaCount) markdown.splice(6, 0, `> 本页包含 ${mediaCount} 个在线课程媒体；离线讲义不复制视频，并在正文对应位置保留资源链接。`, "");
  const mediaNote = mediaCount ? `<aside class="media-note">本页包含 ${mediaCount} 个在线课程媒体。离线讲义不复制视频，并在正文对应位置保留资源链接。</aside>` : "";
  const html = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${escapeHtml(title)}</title><style>${printCss}.media-note{padding:12px 15px;background:#eef5ff;border-left:3px solid #4b78b8;margin:1em 0}.original-title{margin-top:-.5em;color:#777;font-size:1rem;font-weight:400}</style></head><body><main><h1>${escapeHtml(title)}</h1><p class="original-title" lang="en">${escapeHtml(titleInfo.original)}</p><p class="meta">来源：<a href="${canonicalUrl()}">${canonicalUrl()}</a><br>归档时间：${new Date().toLocaleString()}</p>${mediaNote}${htmlBlocks.join("\n")}</main></body></html>`;
  return { id: info.pageId, courseId: info.cardId, chapterPath: info.pathChapter, title, originalTitle: titleInfo.original, url: canonicalUrl(), savedAt: Date.now(), blockCount: blocks.length + 1, mediaCount, markdown: markdown.join("\n"), html };
}
...
async function exportLibrary(requestedCourseId = courseInfo().slug) {
  ...
  const pageById = new Map(pages.map((page) => [String(page.id), page]));
  const itemTitle = (item) => pageById.get(String(item.id))?.title || item.title;
  ...
  if (!pages.length && !courses.length) throw new Error(`当前课程“${requestedCourseId}”尚无已保存目录或讲义`);
  const course = mergeCourseCatalogs(courses, pages);
  const isExercise = (item) => /^(problem|exercise)$/i.test(item.type || "");
  const exerciseCount = course.chapters.flatMap((chapter) => chapter.items || []).filter(isExercise).length;
  const tocMd = ["# 课程目录", "", "> ✓ 双语讲义　↗ 练习题链接　○ 尚未归档", ""];
  course.chapters.forEach((chapter, chapterIndex) => {
    tocMd.push(`## ${chapterIndex + 1}. ${chapter.title}`, "");
    if (chapter.items.some((item) => pageById.has(String(item.id)))) tocMd.push(`[进入本章](#${anchorId("chapter", chapterIndex + 1)})`, "");
    chapter.items.forEach((item, itemIndex) => {
      const number = `${chapterIndex + 1}.${itemIndex + 1}`;
      if (pageById.has(String(item.id))) tocMd.push(`- ✓ [${number} ${itemTitle(item)}](#${anchorId("article", item.id)})`);
      else if (isExercise(item)) tocMd.push(`- ↗ [${number} ${item.title}](${item.url}) — 练习题`);
      else tocMd.push(`- ○ ${number} ${item.title} — 尚未归档`);
    });
    tocMd.push("");
  });
  ...
  const tocHtml = `<nav id="toc" class="toc"><div class="section-label">CONTENTS</div><h1>课程目录</h1><p class="legend"><span>✓ 双语讲义</span><span>↗ 练习题链接</span><span>○ 尚未归档</span></p>${course.chapters.map((chapter, chapterIndex) => { const chapterId = anchorId("chapter", chapterIndex + 1); const label = `<span>${String(chapterIndex + 1).padStart(2, "0")}</span>${escapeHtml(chapter.title)}`; const heading = chapter.items.some((item) => pageById.has(String(item.id))) ? `<a href="#${chapterId}">${label}</a>` : label; return `<section class="toc-chapter"><h2>${heading}</h2><ol>${chapter.items.map((item, itemIndex) => { const number = `${chapterIndex + 1}.${itemIndex + 1}`; if (pageById.has(String(item.id))) return `<li class="ready"><a href="#${anchorId("article", item.id)}"><b>✓</b><span>${number} ${escapeHtml(itemTitle(item))}</span></a></li>`; if (isExercise(item)) return `<li class="exercise"><a href="${escapeHtml(item.url)}" target="_blank" rel="noopener"><b>↗</b><span>${number} ${escapeHtml(item.title)}</span><small>练习题</small></a></li>`; return `<li class="missing"><b>○</b><span>${number} ${escapeHtml(item.title)}</span><small>尚未归档</small></li>`; }).join("")}</ol></section>`; }).join("")}</nav>`;
  ...
  const subtitle = pages.length ? `中英双语课程讲义 · ${pages.length} 篇讲义 · ${exerciseCount} 道练习题` : `练习题链接集 · ${exerciseCount} 道练习题`;
  const html = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${title}</title><style>${css}</style></head><body><header class="cover"><div class="brand">BLOSSOM LINGO</div><h1>${title}</h1><p class="subtitle">${subtitle}</p><p class="meta">个人学习目录 · 原始链接仍受 LeetCode 访问权限约束<br>${new Date().toLocaleDateString()}</p></header>${tocHtml}${articles}</body></html>`;
  const baseName = safeName(course?.title || "leetcode-explore");
  const suffix = pages.length ? "bilingual" : "exercise-links";
  const markdownName = `${baseName}-${suffix}.md`;
  ...
  progress(`已导出 ${pages.length} 篇讲义和 ${exerciseCount} 道练习题链接`, "done");
  return { pageCount: pages.length, exerciseCount, markdownName, htmlName };
}
